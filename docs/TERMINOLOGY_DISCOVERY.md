...
8. **LLM 逐项裁决**：每批最多 4 项，提示词要求对每个候选 ID 明确 keep/drop，drop 也必须
   引用有效证据。模型漏答、格式无效或输出截断时，只重试缺项，批次逐步缩小至单项；
   每项每次续跑最多尝试 4 次，输出预算由 4,000 提至 8,000 token。连接异常时停止本次
   复核，保留已完成结果。运行记录保存逐项决策覆盖与精简错误诊断。
...
旧配置首次加载时复制原草译绑定作为初始值，保存后独立持久化。术语库显示下一次复核
使用的模型；历史运行仍保留最初模型，续跑模型另记在 coverage 中。切换模型只影响未完成项。批准后的术语语境核验使用草译模型。

## 成本、覆盖与失败语义

- 初次本地阶段扫描全文，候选与证据在首次模型请求前一同落库；同一原文和扫描参数复用
  已有运行。译文进度、模型或计算模式变化不触发重扫，风险排序保留扫描时的快照。
- 默认上限为本地 40 项、模型 40 项；模型调用每批最多 4 项，实际池通常小于上限。
- 每次模型回复在同一事务保存新判断及累计 prompt/completion/reasoning token、费用、
  调用数和无效提案数。缺失项从持久化候选重新计算，费用只按新回复累加。
- 复核不完整时运行状态为 `partial`，已有候选继续可见。点击“继续复核 N 项”沿用同一
  run/candidate/evidence ID，仅提交尚未得到有效判断的候选；重复点击由数据库原子占用去重。
- 浏览器请求断开不会取消后台复核；应用重启后已保存的扫描恢复为可续跑状态。旧版
  `completed + error` 记录也能直接续跑，不需要数据库迁移或重新扫描。
- 已完成模型判断以及人工批准、驳回、编辑、撤销批准的记录均受保护；模型调用期间发生
  的人工修改也优先保留。没有拿到有效结果的候选不会被静默当成保留或舍弃。
- 模型 drop 项从默认人工队列折叠，但证据、理由和运行历史全部保留。
- 诊断仅保存候选 ID、尝试序号、输出上限、finish_reason 和错误类别，不保存完整模型回复。

## API

- `POST /documents/{id}/term-discovery-runs`：复用已有全文扫描；无可复用结果时才新建扫描，可选模型复核。
- `POST /documents/{id}/term-discovery-runs/{run_id}/retry`：仅复核未完成项；空请求体沿用已保存模型，可指定 provider/model/compute_mode；不扫描全文。
