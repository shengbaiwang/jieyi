...
并发只影响调度；请求由 `workflow.requests.prepare_translation` 统一准备。
单 SourceAtom 的外层包装由程序恢复，多 SourceAtom 与内联保护仍严格验收。
计算档位保持用户配置，初始输出预算额外为推理预留空间，减少空转重试。
...
`jieyi.prompting.build_messages()` 是 OpenAI-compatible 调用与 prompt preview 的共同来源，避免“界面预览”和“实际请求”漂移。预览同时列出被保护的原文片段、由本地恢复的外层包装与相关术语，但不会发起模型请求。
默认预览对应 CLI/普通执行器；API 的 `?optimized=true` 对应 Web 并发执行器，
与实际执行共用 `prepare_translation`，并返回该段的初始输出上限。

## Token 效率与逐次诊断

详见 [TOKEN_EFFICIENCY.md](TOKEN_EFFICIENCY.md)。优化优先移除可由程序无损恢复的
协议包装与无关结构说明，不删减正文、已命中术语或原有邻段上下文，不自动降低计算档位。
优化执行器对已返回的每次请求记录 `model_call` 审计事件，包含长度、用量、档位、
上限、完成原因及重试序号，不保存正文或完整模型响应。任务进度中的请求/重试/修复数
只统计这些新增记录；历史调用不推算、不补记。token 总量仍取原有批次/旧候选汇总，
不叠加审计事件，推理也不再次叠加到输出。

有可见文本但上游明确标记截断的响应，不能直接验收。预算允许时有限重试，否则隔离
为待复核段；多段结构校验、术语 QA、人工确认保护与断点恢复规则继续生效。
