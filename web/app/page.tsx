...
type Job = {
  ...
  recipe: {
    task_mode?: "draft" | "review"; batch_size?: number; concurrency?: number; max_concurrency?: number;
...
function statusLabel(status: Segment["status"]) { return status === "human_confirmed" ? "已确认" : status === "machine_translated" ? "机器草译" : "待翻译"; }
...
export default function Home() {
  ...
  const currentChapter = useMemo(() => overview?.chapters.find((item) => ordinal >= item.start_ordinal && ordinal <= item.end_ordinal) || null, [ordinal, overview]);
  ...
  const warningIssues = useMemo(() => issues.filter((item) => item.severity !== "error"), [issues]);
  ...
  const openDocument = useCallback(async (document: Document, targetOrdinal?: number) => {
    ...
    try {
      ...
      const [termItems, issueItems, jobItems] = await Promise.all([
        api<Term[]>(`/projects/${summary.project.id}/terms`).catch(() => []), api<Issue[]>(`/documents/${document.id}/issues`).catch(() => []), api<Job[]>(`/documents/${document.id}/jobs`).catch(() => []),
      ]);
      setOverview(summary); setTerms(termItems); setIssues(issueItems); setJobs(jobItems); setJobsDocumentId(document.id);
  ...
  useEffect(() => {
    ...
    function handleEpubResize(event: MessageEvent) {
      const data = event.data as { type?: string; documentId?: string; spineIndex?: number; height?: number };
      if (data?.type !== "jy-epub-resize" || data.documentId !== epubReader?.document_id) return;
      const spineIndex = Number(data.spineIndex);
      const reportedHeight = Number(data.height);
      if (!Number.isInteger(spineIndex) || !Number.isFinite(reportedHeight)) return;
    ...
    window.addEventListener("message", handleEpubResize);
    return () => window.removeEventListener("message", handleEpubResize);
  }, [epubReader]);
    ...
    async function pollJob() {
      try {
        ...
        if (overview?.document.id === jobsDocumentId) {
          const [summary, issueItems] = await Promise.all([
            api<Overview>("/documents/" + jobsDocumentId + "/overview"),
            api<Issue[]>("/documents/" + jobsDocumentId + "/issues"),
          ]);
          if (!disposed) { setOverview(summary); setIssues(issueItems); }
        }
      } catch { return; }
    }
  ...
  async function openReader(document: Document) {
    ...
    try {
      ...
      const items = await api<Segment[]>("/documents/" + document.id + "/segments");
      legacySegments.current.set(document.id, items);
      setReaderSegments(items);
      if (document.source_format === "epub") {
        setEpubReader(
          await api<EpubReaderManifest>("/documents/" + document.id + "/epub")
            .catch(() => null)
        );
      } else {
        setEpubReader(null);
    ...
    try {
      ...
      if (!job) {
        job = await api<Job>("/documents/" + document.id + "/jobs", { method: "POST", body: JSON.stringify({
          ...
          review_policy: mode === "review" ? "on_issue" : "never",
          ...
          draft_compute_mode: currentSettings.draft_compute_mode, review_compute_mode: currentSettings.reviewer_compute_mode, review_sample_rate: 0.08,
    ...
    <section className={`workspace-window ${!sidebarOpen ? "sidebar-closed" : ""} ${!inspectorOpen || panel !== "translate" ? "inspector-closed" : ""}`} aria-label="介译翻译工作台">
      ...
      <div className="workspace-grid">
        <aside className="sidebar"><div className="sidebar-scroll"><div className="eyebrow">工作空间</div><nav aria-label="工作空间">
          <button className={`nav-item ${panel === "library" ? "active" : ""}`} onClick={() => setPanel("library")}><Mark>▦</Mark>书库 <b>{documents.length}</b></button>
          <button className={`nav-item ${panel === "translate" ? "active" : ""}`} disabled={!overview} onClick={() => setPanel("translate")}><Mark>▤</Mark>阅读与翻译</button>
          <button className={"nav-item " + (panel === "reader" ? "active" : "")} disabled={!overview} onClick={() => overview?.document && void openReader(overview.document)}><Mark>☷</Mark>阅读模式</button>
          ...
          <button className={`nav-item ${panel === "quality" ? "active" : ""}`} disabled={!overview} onClick={() => setPanel("quality")}><Mark>◉</Mark>质量检查 {errorIssues.length > 0 && <b className="issue-count">{errorIssues.length}</b>}</button>
        ...
        {overview && <><div className="eyebrow section-label"><span>目录</span><small>{overview.chapters.length} 节</small></div><div className="chapter-list real-chapters">{overview.chapters.map((chapter, index) => <button key={`${chapter.start_ordinal}-${chapter.level}-${chapter.title}`} data-level={chapter.level} style={{ paddingLeft: `${9 + Math.min(chapter.level, 3) * 12}px` }} className={`chapter ${currentChapter?.start_ordinal === chapter.start_ordinal ? "active" : ""}`} onClick={() => { if (panel === "reader") document.getElementById("reader-segment-" + chapter.start_ordinal)?.scrollIntoView({ behavior: "smooth" }); else void navigateTo(chapter.start_ordinal); }} title={chapter.title}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{chapter.title}</strong><small>{chapter.confirmed_count} 已确认 · {chapter.segment_count} 段</small></div></button>)}</div></>}
