...
首次启动的书库为空，可以导入自己的 PDF/EPUB/TXT/Markdown，或用 `examples/sample.md` 体验。CLI 内置 `echo` 供应商可离线验证流程；真实翻译需要配置模型服务。macOS 支持钥匙串保存密钥，其他平台可使用环境变量或仅在本次运行会话中输入密钥。
...
- 在左侧选择“导入书籍”，拖入 PDF、EPUB、TXT、MD 或 Markdown 文件，确认项目名称、书名、原文/目标语言和翻译风格，检查分段预览后点击“导入并创建项目”；
...
术语库支持三档采用方式：**参考译法**仅供翻译时参考，可根据上下文改译，不触发一致性检查；**按义项采用**在相应义项适用时要求采用；**固定译法**要求所有命中用法统一。可在添加术语时选择，也可直接在术语列表中切换，已有译文的质量提示会随之更新。CLI 可用 `term-add --enforcement reference` 添加参考译法。
...
支持带文字层的 PDF（最大 128 MB、2,000 页）。导入面板展示逐页解析进度，
读取书名与书签，整理重复页眉页脚、连字、段内断行和明确的跨页续段。
预览解析结果会短暂缓存，确认导入时复用；重复导入同一项目不会新增副本。
原文件与分段在同一数据库事务中保存。

导入时会结合字形位置、字体、标题对齐、正文缩进和实际行距重建段落，
将多行标题、放大或下沉的段首字母及目录中的标题续行合回同一单元。
新导入的 PDF 自动使用当前分段器；已有 PDF 可运行
`uv run jieyi pdf-resegment --document 文档ID` 更新分段。
升级保留能按原文和页码准确匹配的段落 ID、译文及校对记录；如涉及无法匹配的已有译文、
手工分段或未完成翻译任务，则停止并保留原文档。升级前建议备份数据库。

PDF 与其他书稿共用单段/选章/全书草译、术语、人工校对、自动保存与双语阅读。
点击段落旁的“原页”可查看对应 PDF 页面，支持输入页码、前后翻页、放大和下载原件。
原页面板使用按需渲染的静态图片，不执行 PDF 中的脚本或附件。
阅读模式提供原文、译文、双语对照，以原页呈现封面、插图、图表、页边距与页面尺寸。
“导出书籍”生成 PDF：在能安全回填的原文字区域内排入译文，保留原生图片与矢量图形；
按可见字形匹配原段落，忽略缩进与尾部空格；原字框容纳不下时，使用原栏内与相邻内容之间的空白，
优先保持原字号，再适度缩小，避免覆盖相邻文字和图形。跨页段落统一排版，预览与导出保持一致。
未译内容仍显示原文。双语 PDF 逐页交替排列原页和译页。
译文过长、与图形相邻、旋转页或人工拆并段后无法直接回填原框时，会在同一物理页的
相关文字区域内整体重排并完整容纳译文；无法在原页安全排下时停止导出并明确报错，不会遗漏或增加续页。
仍可单独导出纯文本。中文排版嵌入本机中文字体；macOS 自动使用黑体，
其他系统可通过 `JIEYI_PDF_FONT` 指定可嵌入的中文 TTF/TTC 字体。

扫描版须先做 OCR；混合文件会提示文字较少的页数，保留原页供核对。
复杂表格、多栏和脚注的文字顺序仍需要人工核查，工作台支持编辑与拆并段。

API：`POST /imports/pdf/inspect` 接收原始 PDF 字节并返回解析 ID，
`GET /imports/pdf/inspect/{id}` 返回进度、预览或可操作的错误提示；
`POST /projects/{project_id}/documents/pdf` 导入原始字节。
`GET /documents/{id}/pdf` 返回目录和页码元数据，
`GET /documents/{id}/pdf/pages/{page}` 返回静态原页（`mode=translated` 返回译页），
`GET /documents/{id}/pdf/original` 下载原件。
